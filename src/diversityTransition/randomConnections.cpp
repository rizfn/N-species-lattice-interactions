...
constexpr double DEFAULT_GROWTH_RATE = 0.00001;
constexpr double REACTION_RATE_MIN = 0.5;
constexpr double REACTION_RATE_MAX = 1.0;
constexpr double SPARSITY = 0.8;

void set_random_ICs(int N_RESOURCES, int N_CHEMICALS, int n_records, std::vector<std::vector<double>> &S_vals, std::vector<std::vector<double>> &X_vals, std::mt19937 &gen)
{
    std::uniform_real_distribution<> dis(0.0, 1.0);

    S_vals.assign(N_RESOURCES, std::vector<double>(n_records, 0.0));
    X_vals.assign(N_CHEMICALS, std::vector<double>(n_records, 0.0));
...
void ode_integrate_rk4(int N_RESOURCES,
                       ...
                       int dataskip,
                       std::vector<double> &T,
                       std::vector<std::vector<double>> &S_vals,
                       ...
                       std::mt19937 &gen)
{
    ...
    set_random_ICs(N_RESOURCES, N_CHEMICALS, n_records, S_vals, X_vals, gen);
    T.assign(n_records, 0.0);

    std::vector<double> current_S(N_RESOURCES), current_X(N_CHEMICALS);
    for (int i = 0; i < N_RESOURCES; ++i)
        current_S[i] = S_vals[i][0];
    for (int i = 0; i < N_CHEMICALS; ++i)
        current_X[i] = X_vals[i][0];
    ...
    for (int i = 0; i < nsteps; ++i)
    {
        ...
        {
            for (int j = 0; j < N_RESOURCES; ++j)
                S_vals[j][record_idx] = current_S[j];
            for (int j = 0; j < N_CHEMICALS; ++j)
                X_vals[j][record_idx] = current_X[j];
...
void run_simulation(std::ofstream &file, int N_RESOURCES, int N_CHEMICALS, double growth_rate, int N_STEPS, int dataskip, const std::vector<std::vector<int>> &connectivity, const std::vector<std::vector<double>> &reaction_rates)
{
    std::random_device rd;
    std::mt19937 gen(rd());

    std::vector<double> T;
    std::vector<std::vector<double>> S_vals, X_vals;
    ode_integrate_rk4(N_RESOURCES, N_CHEMICALS, connectivity, reaction_rates, growth_rate, N_STEPS, N_STEPS, dataskip, T, S_vals, X_vals, gen);
...
std::pair<std::vector<std::vector<int>>, std::vector<std::vector<double>>> init_connectivity_and_reaction_rates(int N_RESOURCES, int N_CHEMICALS, double sparsity, std::mt19937 &gen)
{
    ...
    std::uniform_real_distribution<> dis(0.0, 1.0);
    std::uniform_real_distribution<> rate_dis(REACTION_RATE_MIN, REACTION_RATE_MAX);

    for (int i = 0; i < N_CHEMICALS; ++i)
    {
        for (int j = 0; j < N_CHEMICALS; ++j)
        {
            if (dis(gen) >= sparsity && i != j)
            {
                connectivity[i][j] = std::uniform_int_distribution<>(0, N_RESOURCES - 1)(gen);
                reaction_rates[i][j] = rate_dis(gen);
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 4)
        N_STEPS = std::stoi(argv[4]);
    if (argc > 5)
        N_sims = std::stoi(argv[5]);
    ...
    {
        threads.push_back(std::thread([sim, N_RESOURCES, N_CHEMICALS, growth_rate, N_STEPS, dataskip, connectivity, reaction_rates, exeDir]()
        {
            ...
            std::ofstream file;
            file.open(filePath);
            run_simulation(file, N_RESOURCES, N_CHEMICALS, growth_rate, N_STEPS, dataskip, connectivity, reaction_rates);
            file.close();
        }));
    }
