...
def run_simulation(growth_rate, stoptime, nsteps, connectivity, reaction_rates, S0, X0, N_s, N_c, dataskip, output_dir):
    ...
    plt.savefig(f'{output_dir}/gamma_{growth_rate}.png', dpi=300)
    plt.close()
...
def main():
    ...
    growth_rates = np.geomspace(1e-4, 1e2, 7)
    stoptimes = [100_000_000 ,10_000_000, 10_000_000, 10_000_000, 1_000_000, 1_000_000, 100_000]
    nsteps = 10_000_000

    dataskip = 100

    output_dir = f'src/diversityTransitionScaleFreeNetwork/plots/sameNetwork/N_{N_s}-{N_c}_exponent_{power_law_exponent}/'

    sim_args = [(growth_rates[i], stoptimes[i], nsteps, connectivity, reaction_rates, S0, X0, N_s, N_c, dataskip, output_dir) for i in range(len(growth_rates))]

    with multiprocessing.Pool() as pool:
        pool.starmap(run_simulation, sim_args)
