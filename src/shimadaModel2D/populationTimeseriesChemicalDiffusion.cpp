...
constexpr float DEFAULT_D = 1.0f;       // diffusion constant
    ...
    if (site_value != EMPTY)
    {
        ...
        else
        {
            ...
            if (new_site_value == EMPTY)
            {
                ...
                {
                    replicationProbability += chemical_lattice[chemical][new_site.first][new_site.second];
...
void run(std::ofstream &file, int L, int N_SPECIES, int N_CHEMICALS, double THETA, int K, int STEPS_PER_LATTICEPOINT)
{
    ...
    for (long long step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        for (int i = 0; i < L * L; ++i)
        {
            update_bacteria(bacteria_lattice, chemical_lattice, J, S, BACTERIA, L, THETA, K, dis_l, dis_real, gen);
        }
...
int main(int argc, char *argv[])
{
    ...
    for (int run_idx = 0; run_idx < N_runs; ++run_idx)
    {
        threads.push_back(std::thread([run_idx, N_SPECIES, N_CHEMICALS, L, THETA, K, STEPS_PER_LATTICEPOINT, exeDir]()
                ...
                filePathStream << exeDir << "/outputs/timeseriesChemicalDiffusion/N_" << N_SPECIES << "-" << N_CHEMICALS << "_L_" << L << "_theta_" << THETA << "_K_" << K << "_" << run_idx << ".csv";
