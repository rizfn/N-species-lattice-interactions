...
constexpr int DEFAULT_N_SPECIES = 50;
constexpr int DEFAULT_N_CHEMICALS = 20;
                       ...
                       std::mt19937 &gen)
{
    ...
    {
        ode_derivatives(current_Os, current_Ns, J, S, THETA, k1_Os, k1_Ns);
        ...
        ode_derivatives(O_temp, N_temp, J, S, THETA, k2_Os, k2_Ns);

        for (int j = 0; j < N_SPECIES; ++j)
            O_temp[j] = current_Os[j] + 0.5 * dt * k2_Os[j];
        for (int j = 0; j < N_CHEMICALS; ++j)
            N_temp[j] = current_Ns[j] + 0.5 * dt * k2_Ns[j];
        ode_derivatives(O_temp, N_temp, J, S, THETA, k3_Os, k3_Ns);

        for (int j = 0; j < N_SPECIES; ++j)
            O_temp[j] = current_Os[j] + dt * k3_Os[j];
        for (int j = 0; j < N_CHEMICALS; ++j)
            N_temp[j] = current_Ns[j] + dt * k3_Ns[j];
        ode_derivatives(O_temp, N_temp, J, S, THETA, k4_Os, k4_Ns);
...
void run_simulation(std::ofstream &file, int N_SPECIES, int N_CHEMICALS, double THETA, int N_STEPS, int dataskip, const std::map<int, std::vector<int>> &J, const std::map<int, std::vector<int>> &S)
{
    ...
    ode_integrate_rk4(N_SPECIES, N_CHEMICALS, J, S, THETA, N_STEPS, N_STEPS, dataskip, T, Os, Ns, gen);
...
int main(int argc, char *argv[])
{
    ...
    {
        threads.push_back(std::thread([sim, N_SPECIES, N_CHEMICALS, THETA, N_STEPS, dataskip, J, S, exeDir]()
        {
            std::ostringstream filePathStream;
            filePathStream << exeDir << "/simulation_" << std::setw(2) << std::setfill('0') << sim << ".csv";
            std::string filePath = filePathStream.str();

            std::ofstream file;
            file.open(filePath);
            run_simulation(file, N_SPECIES, N_CHEMICALS, THETA, N_STEPS, dataskip, J, S);
            file.close();
        }));
    }
