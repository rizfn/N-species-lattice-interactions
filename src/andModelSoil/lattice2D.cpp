...
constexpr int DEFAULT_N_RUNS = 1;       // number of parallel runs
...
std::vector<std::vector<int>> init_bacteria_lattice(int L, int N_SPECIES, std::mt19937 &gen)
{
    ...
    std::generate(sites.begin() + soil_sites, sites.end(), [&]()
                  { return dis_bacteria(gen); });
...
void run(int L, int N_SPECIES, int N_CHEMICALS, double THETA, double SIGMA, int K_IN, int K_OUT, int STEPS_PER_LATTICEPOINT, int FINAL_STEPS_TO_RECORD, double D, const std::string &dirPath, int run_idx)
{
    ...
    // Open lattice recording file
    std::ostringstream latticeFilePathStream;
    latticeFilePathStream << dirPath << "/lattice_" << run_idx << ".tsv";
    std::ofstream latticeFile(latticeFilePathStream.str());
    ...
    for (long long step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        ...
        // Record the lattice only for the final steps
        if (step >= STEPS_PER_LATTICEPOINT - FINAL_STEPS_TO_RECORD)
        {
...
int main(int argc, char *argv[])
{
    ...
    for (int run_idx = 0; run_idx < N_runs; ++run_idx)
    {
        threads.push_back(std::thread([run_idx, N_SPECIES, N_CHEMICALS, L, THETA, SIGMA, D, K_IN, K_OUT, STEPS_PER_LATTICEPOINT, exeDir]()
                ...
                dirPathStream << exeDir << "/outputs/lattice2D/N_" << N_SPECIES << "-" << N_CHEMICALS << "_L_" << L << "_theta_" << THETA << "_K_IN_" << K_IN << "_K_OUT_" << K_OUT << "_D_" << D << "_sigma_" << SIGMA;
                std::string dirPath = dirPathStream.str();
                std::filesystem::create_directories(dirPath);

                run(L, N_SPECIES, N_CHEMICALS, THETA, SIGMA, K_IN, K_OUT, STEPS_PER_LATTICEPOINT, 1000, D, dirPath, run_idx); }));
    }
