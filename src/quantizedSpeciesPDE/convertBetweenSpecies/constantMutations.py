...
@njit
def run_simulation(t, x, dt, dx, D_N, D_S, boundary_N, g, N_species, mu, save_interval):
    """Run the main simulation loop with semi-implicit diffusion and mutation."""
...
def main():
    ...
    save_interval = int(100.0 / dt)  # Save every 1 time unit
    
    print("Running simulation...")
    species_history, nutrients_history, final_species, final_nutrients = run_simulation(
        t, x, dt, dx, D_N, D_S, boundary_N, g, N_species, mu, save_interval
    )
    ...
    plt.tight_layout()
    os.makedirs(f"src/quantizedSpeciesPDE/convertBetweenSpecies/plots/constantMutations/N{N_species}_DN{D_N}_DS{D_S}_mu{mu}", exist_ok=True)
    with tqdm(total=len(time_points), desc="Saving animation") as pbar:
        def progress_callback(current_frame, total_frames):
            pbar.update(1)

        anim.save(f"src/quantizedSpeciesPDE/convertBetweenSpecies/plots/constantMutations/N{N_species}_DN{D_N}_DS{D_S}_mu{mu}/animation.mp4",
    ...
    plt.title(f'Growth Rate Functions for All Species (μ={mu})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"src/quantizedSpeciesPDE/convertBetweenSpecies/plots/constantMutations/N{N_species}_DN{D_N}_DS{D_S}_mu{mu}/growth_functions.png")
    ...
    plt.xlabel('Position')
    plt.ylabel('Density')
    plt.title(f'Final State: Species and Nutrients (μ={mu})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"src/quantizedSpeciesPDE/convertBetweenSpecies/plots/constantMutations/N{N_species}_DN{D_N}_DS{D_S}_mu{mu}/final_state.png")
