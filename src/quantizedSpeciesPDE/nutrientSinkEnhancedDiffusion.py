...
def main():
    ...
    for time_idx, time in enumerate(tqdm(t, desc="Running simulation")):
        ...
        # Update each species
        for i in range(N_species):
            # Calculate nutrient-dependent diffusion coefficient
            D_S_local = D_S * nutrients  # Diffusion proportional to local nutrient concentration
            
            # Species diffusion with spatially variable diffusion coefficient
            # Need to use the divergence form: d/dx(D(x) * dS/dx)
            dS_dx = np.zeros_like(species[i])
            d2species_dx2 = np.zeros_like(species[i])
            
            # Calculate first derivative dS/dx using central differences
            dS_dx[1:-1] = (species[i, 2:] - species[i, :-2]) / (2 * dx)
            dS_dx[0] = (species[i, 1] - species[i, 0]) / dx  # Forward difference at left
            dS_dx[-1] = (species[i, -1] - species[i, -2]) / dx  # Backward difference at right
            
            # Calculate d/dx(D * dS/dx) for interior points
            D_dS_dx = D_S_local * dS_dx
            d2species_dx2[1:-1] = (D_dS_dx[2:] - D_dS_dx[:-2]) / (2 * dx)
            
            # Boundary conditions with variable diffusion - zero flux
            # Left boundary: For zero flux, assume dS/dx = 0 at boundary
            d2species_dx2[0] = (D_S_local[1] * (species[i, 1] - species[i, 0]) / dx) / dx
            
            # Right boundary: At sink (N=0), D_S_local[-1]=0, so no diffusion flux
            # For zero flux condition at boundary, we need the flux from interior to be zero
            # Since D_S_local[-1] = 0, the natural condition is already zero flux
            d2species_dx2[-1] = 0  # No diffusion term at sink boundary
            
            # Growth term with local dilution
            growth = R_values[i] * species[i] - species[i] * local_dilution
            
            # Update species: variable diffusion + growth
            species[i] += (d2species_dx2 + growth) * dt
    ...
    plt.tight_layout()
    os.makedirs("src/quantizedSpeciesPDE/plots/nutrientSinkEnhancedDiffusion", exist_ok=True)
    with tqdm(total=len(time_points), desc="Saving animation") as pbar:
        def progress_callback(current_frame, total_frames):
            pbar.update(1)
        
        anim.save(f"src/quantizedSpeciesPDE/plots/nutrientSinkEnhancedDiffusion/animation_N{N_species}_DN{D_N}_DS{D_S}.mp4", 
    ...
    plt.tight_layout()
    plt.savefig(f"src/quantizedSpeciesPDE/plots/nutrientSinkEnhancedDiffusion/growth_functions_N{N_species}_DN{D_N}_DS{D_S}.png")
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"src/quantizedSpeciesPDE/plots/nutrientSinkEnhancedDiffusion/final_state_N{N_species}_DN{D_N}_DS{D_S}.png")
