...
// Constants
constexpr int N = 2; // number of species/nutrients
    ...
    for (int k = 0; k < N; ++k)
    {
        for (int i = 0; i < L; ++i)
        {
            for (int j = 0; j < L; ++j)
            {
                ...
                // i-direction (vertical/y): no-flux boundary conditions (gradient allowed)
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "/outputs/chainLatticeSnapshots/N_" << N << "_L_" << L
                   << "_theta_" << THETA << "_D_" << D << "_S_" << S << ".tsv";
