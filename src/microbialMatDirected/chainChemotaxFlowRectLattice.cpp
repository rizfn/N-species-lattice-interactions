...
constexpr int L_x = 32;        // lattice width (horizontal)
constexpr int L_y = 128;       // lattice height (vertical)
constexpr float D = 0.25f;      // diffusion constant
constexpr float DRIFT = 0.5f;   // vertical drift parameter
constexpr float S = 100.0f;      // boundary condition (nutrient 0 supply rate)
                                             ...
                                             std::mt19937 &gen)
{
    ...
    std::pair<int, int> neighbors[4] = {
        ...
        {c.first, (c.second - 1 + L_x) % L_x}, // left (periodic in x)
        {c.first, (c.second + 1) % L_x}      // right (periodic in x)
    };
    ...
    if (total_weight == 0.0f)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (neighbors[i].first >= 0 && neighbors[i].first < L_y)
            {
...
{
    std::uniform_int_distribution<> dis_site(0, N); // 0=empty, 1-N=bacteria species
    std::vector<std::vector<int>> bacteria_lattice(L_y, std::vector<int>(L_x));
    for (int i = 0; i < L_y; ++i)
    {
        for (int j = 0; j < L_x; ++j)
        {
...
{
    std::vector<std::vector<std::vector<float>>> chemical_lattice(N, std::vector<std::vector<float>>(L_y, std::vector<float>(L_x, 0.0f)));
...
void supply_nutrient_from_top(std::vector<std::vector<std::vector<float>>> &chemical_lattice)
{
    ...
    for (int j = 0; j < L_x; ++j)
    {
        ...
        // Renormalize this site only if sum > 1
        float total = 0.0f;
        for (int k = 0; k < N; ++k)
        {
            total += chemical_lattice[k][0][j];
        }
...
void diffuse_chemicals(std::vector<std::vector<std::vector<float>>> &chemical_lattice)
{
    ...
    std::vector<std::vector<std::vector<float>>> new_chemical_lattice(N, std::vector<std::vector<float>>(L_y, std::vector<float>(L_x, 0.0f)));
    ...
    for (int k = 0; k < N; ++k)
    {
        for (int i = 0; i < L_y; ++i)
        {
            for (int j = 0; j < L_x; ++j)
            {
                ...
                float left = chemical_lattice[k][i][(j - 1 + L_x) % L_x];
                float right = chemical_lattice[k][i][(j + 1) % L_x];
                
                // i-direction (vertical/y): no-flux boundary conditions
                float up = (i > 0) ? chemical_lattice[k][i - 1][j] : chemical_lattice[k][i][j];
                float down = (i < L_y - 1) ? chemical_lattice[k][i + 1][j] : chemical_lattice[k][i][j];
    ...
    for (int k = 0; k < N; ++k)
    {
        for (int i = L_y - 1; i >= 0; --i) // Process ALL rows including bottom
        {
            for (int j = 0; j < L_x; ++j)
            {
...
{
    std::uniform_int_distribution<> dis_i(0, L_y - 1);
    std::uniform_int_distribution<> dis_j(0, L_x - 1);
    std::uniform_real_distribution<> dis_real(0.0, 1.0);

    // Select a random site
    std::pair<int, int> site = {dis_i(gen), dis_j(gen)};
...
void normalize_chemicals(std::vector<std::vector<std::vector<float>>> &chemical_lattice)
{
    for (int i = 0; i < L_y; ++i)
    {
        for (int j = 0; j < L_x; ++j)
        {
            // Calculate total concentration at this site
            float total = 0.0f;
            for (int k = 0; k < N; ++k)
            {
                total += chemical_lattice[k][i][j];
            }
            
            // Only normalize if total > 1
            if (total > 1.0f)
            {
                for (int k = 0; k < N; ++k)
                {
                    chemical_lattice[k][i][j] /= total;
                }
            }
        }
    }
...
void run(std::ofstream &file)
{
    ...
    for (long long step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        ...
        // Update bacteria L_x*L_y times per step
        ...
        // Normalize chemicals so sum equals 1 at each site
        normalize_chemicals(chemical_lattice);
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "/outputs/chainChemotaxisFlowLattice/N_" << N << "_Lx_" << L_x << "_Ly_" << L_y
